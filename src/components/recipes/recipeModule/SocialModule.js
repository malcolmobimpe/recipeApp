...
class SocialModule extends React.Component {
  ...
  render(){
   ...
   // console.log({recipeId, currentLikes, userId})
...
{ likeList.includes(userId) ? <TiHeart className='recipe-icon'/> : <TiHeartOutline  className='recipe-icon'/>}
</span>
...
const mapDispatchToProps = dispatch => {
  return {
    updatePostLikes: (docId, currentLikes, userId) => dispatch (updatePostLikes(docId, currentLikes, userId)),
    dislikePost: (docId, currentLikes, userId) => dispatch (dislikePost(docId, currentLikes, userId))
